...
### Diagonal roads come out as a notched staircase

**Problem:** Pouring at roughly 45° at ordinary driving speed stepped the road in one-tile jogs, changed its width, and left bare corners. Avoiding it meant crawling.

**Root cause:** The width row was built by `getBackSquares`, which snapped the perpendicular to whichever cardinal axis dominated and grew the row one-sided from the truck (`i = 0..width-1`). Four consequences stacked up. A row of `W` tiles laid on a cardinal axis spans only about `W × cos45°` across a diagonal, and the overhang became the steps. Near 45° the two perpendicular components are nearly equal, so steering wobble flipped which axis dominated and swung the one-sided row to the truck's other side. `UPDATE_INTERVAL` is 0.5s, so at any speed above a crawl every tick jumped several tiles and ran the Bresenham path, which stamped that same cardinal row at each interpolated point and reproduced the staircase. Finally `applySmoothRoad` treated the row's first and last entries as the road's edges, which on a diagonal are the cardinal ends of a staircase step.

**Fix:** A pour tick now rasterises the area swept between the previous and current pour points. `DumpTruckBandRaster.getColumns` covers a tile when its centre lies inside that swept rectangle, so the geometry does not care how the road lies on the grid, and the same sweep serves a crawl and a truck at speed. See design-notes "Road geometry — the swept band" for the half-open width test, the even-width bias, and the column ordering.

Sampling points along the perpendicular would not have worked: at 45° that line runs through tile corners, so its floored samples land on tiles that touch only at their corners, leaving holes between them. `tests/band_raster_test.lua` asserts a 45° sweep is reachable north/south/east/west from end to end, along with exact cardinal rows and the even-width bias.

Smoothing follows the same split. Gap fillers are checked on every square of a column, because a diagonal's outer hull is a staircase whose pockets sit beside middle squares as readily as end ones. Edge blends border the column's two end squares, facing outward along the column's own across vector, which the multiplayer server can read from the bare coordinates it receives.

### Edge blends missing along diagonal roads

**Problem:** Roads poured off the cardinal axes came out hard-edged against the grass. Blends landed correctly on straightaways but went missing in stretches, most visibly beside a gap filler.

**Root cause:** Two causes, both in `addEdgeBlends`. `smoothRoad` ran it only on columns whose squares shared an X or a Y, on the assumption that a diagonal's border is carried by its gap fillers — but gap fillers fill corner pockets, and the long flat face of a staircase step is not a pocket, so nothing bordered it. Steering wobble was enough to tip a column off its axis and skip its blends too, which is why the gaps also showed up on roads that looked straight. Underneath that, the function offered each end of a column exactly one outward direction, chosen by a shared-axis test. An end square on a staircase is exposed on two faces, and when the one direction on offer pointed at a gap filler that had since become gravel, it was spent on a neighbour needing no blend and the genuinely exposed face got nothing.
...
- **One unclean edge blend observed** – Server coords (16360, 702): tile is on the **edge next to a gap filler**; several other gap-filler edges are fine, this was the only one. **Note:** Something about this spot left a blend we didn't clean. The suspected cause — cleanup skipping row ends, which is where a gap-filler-adjacent square usually sits — no longer holds: `smoothRoad` runs `removeOppositeEdgeBlends` over every square in a column, ends included. Watch for it recurring.
- **Dump loop sounds low and rumbling** – Observed in play: the gravel loop reads as a low rumble, missing the high end where the pouring gravel lives. The loop plays `media/sound/new_gravel_loop.ogg` (66 KB) via `GravelDumpLoop` in `sounds_dumptruck.txt`, and the sound folder also carries an unused `gravel_loop_12s.ogg` (116 KB) and `gravel_dump_dry.ogg` — worth comparing the three by ear, since the shipped loop is the leanest of them. Nothing in the pour path touches audio: sound is start, loop and end only, with no per-square clip, and both the script and the ogg files are byte-identical to `42.13`.
...
- **Zoom-based gravel loop volume** – Optional: while the loop is playing, set volume from `getCore():getZoom()` (normalize with min/max) and `vehicle:getEmitter():setVolume(handle, volume)`. The loop handle is `DumpTruck.dumpLoopHandle`.
...
- **Dump loop stays at one map tile after rapid Start Dumping (MP)** – Workshop and local dedicated: mashing Start Dumping leaves `GravelDumpLoop` playing from the toggle square after driving away; Stop from that radial can miss it; relog clears it. Radial click uses the live truck; the slice is Start or Stop from `DumpTruck.dumpingActive` at menu open. Stop on that truck is `stopSound(DumpTruck.dumpLoopHandle)` on that emitter. Unreproduced here. Verify on this branch, then move to Resolved.
